<template>
  <q-page class="p-6 max-w-5xl mx-auto">
    ...
    <q-inner-loading :showing="songStore.loading && !song && !loadError">
      <q-spinner
        size="40px"
        color="primary"
      />
    ...
    <template v-if="song">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-grey-6">
              {{ song.albums?.length || 0 }} album{{ song.albums?.length === 1 ? '' : 's' }}
            </p>
          </div>
        </div>
      ...
      <h2 class="text-xl font-semibold text-grey-8 mb-4">
        Appears In
      </h2>

      <div
        v-if="song.albums?.length === 0"
...
const song = computed(() => songStore.currentItem)
...
function confirmDelete() {
  $q.dialog({
    ...
    try {
      await songStore.remove(song.value.id)
